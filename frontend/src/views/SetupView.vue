...
const apiKey = ref('')
const concurrency = ref(5)   // 并发数：同时进行的 AI 请求数（1-64，默认 5；v1.3.10 上限 16→64 高并发 API 顶满）
...
async function runTest(which) {
  ...
  try {
    ...
    if (!r.ok && apiKey.value === API_KEY_PLACEHOLDER) {
      // 修复（v1.4.2）：keyring 里的 key 被服务器拒绝（401/403）→ 占位符继续显示会让用户
      // 误以为 key 仍有效（「每次打开都是 API Key 无效」的困惑源头）。清掉占位符强制重填。
      apiKey.value = ''
    }
    if (r.ok) {
      // 修复（v1.4.2）：测试连接成功 → 若表单是真实 key（非占位符）立即写入 keyring——
      ...
      autoSave()   // 测试连接成功即保存当前设置（用户改了没点保存也不丢）
    }
  } catch (e) {
    testOk.value = false
...
async function runThroughputTest() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // 修复（v1.4.2）：吞吐测试成功同样验证了 key → 真实 key 立即落盘 keyring，
