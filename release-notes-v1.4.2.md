...
硬编码 AI 判断原逐批串行——`rate_gate.acquire()`（RPM 限速）在并发信号量 `sem` 内部，等令牌时占着并发槽，其他批拿不到 `sem`，实际串行。2 小时才处理 12000 条。

**修复**：
1. `rate_gate.acquire()` 移到 `sem` 外部——先等令牌再拿并发槽，并发批同时等令牌后同时发请求
2. 硬编码阶段临时提高并发（翻译并发 ×2，封顶 32）——每条文本很短，不需要保守并发
3. 完成后恢复原并发
