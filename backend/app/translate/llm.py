        ...
        body = {
            ...
            "messages": [
                {"role": "system", "content":
                    ...
                    f"专有名词（模组名、类名、API 名、命令名）保留原文，其余一律翻译成目标语言，"
                    f"译文必须是通顺的目标语言，读起来像原生中文；禁止把英文单词硬插进中文短语，"
        ...
        body = {
            ...
            "messages": [
                {"role": "system", "content":
                    ...
                    f"%%MC_数字%% 占位符原样；专有名词（模组名/类名/API/命令名）保留原文，"
                    f"其余一律翻译成目标语言，译文通顺、读起来像原生中文；禁止把英文单词"
                    f"硬插进中文短语，能意译的英文一律意译，避免中英混杂；"
                    f"原文含换行时用 \\n 表示，只输出译文，不要解释。"},
