...
# v1.2.9/1.3.0：语言文件值若整体是布尔字面量（`xxx.enabled=true` 的 "true"）是配置显示
# 不是文本，跳过翻译。**只跳 true/false 纯布尔**——原含 enabled/on/none/yes 等词误杀真实
# 按钮文本（"Enabled" 该翻「已启用」却被跳过 → 用户「全英文」），恢复翻译。


def lang_value_ok(value: str) -> bool:
    """语言文件的值是否值得翻译：仅「长度 2-500 + 含字母/汉字」，且非纯布尔字面量。

    语言文件的值本就是可翻译文本（键才是标识符），因此不走 should_translate
    的技术标识符规则——"Requires_Armor" 这类 snake_case 形态是真实英文短语，
    不能被当 iron_ingot 那样的技术串滤掉。只排除过短/纯数字/纯符号/纯布尔。
