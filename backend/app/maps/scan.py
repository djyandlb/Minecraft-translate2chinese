...
def scan_nbt(data, keys: set[str], acc: list, path_str: str = "") -> None:
    ...
    if kind == "compound":
        for k, v in items:
            ...
            if val is not None:
                ...
                scan_nbt(v, keys, acc, child)
    elif kind == "list":
        for i, item in enumerate(items):
            scan_nbt(item, keys, acc, f"{path_str}[{i}]")
...
def scan_mcfunction(path: Path, keys: set[str]) -> list[dict]:
    """.mcfunction 扫描：say/tellraw/title 后的引号文本。"""
    acc: list[dict] = []
    try:
        ...
        for cx in range(32):
            ...
            for key in ("TileEntities", "block_entities"):
                try:
                    ...
                    continue
                scan_nbt(be_list, keys, acc, f"chunk({cx},{cz})")
    return acc
